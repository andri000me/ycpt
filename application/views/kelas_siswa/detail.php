					...
					<div class="row">
						<div class="col-lg-12">
							<form role="form" action="<?php echo base_url(); ?>kelas_siswa/save_siswa" method="post" enctype="multipart/form-data" name="form" class="form" id="form" onsubmit="return validate(this)" onClick="highlight(event)" onKeyUp="highlight(event)">
								...
								<input type="hidden" class="form-control" value="<?php echo $m->id_kelas; ?>" name="id_kelas" placeholder="ID">
								<input type="hidden" class="form-control" value="<?php echo $m->id_tahun_ajaran; ?>" name="id_tahun_ajaran" placeholder="ID">
								<div class="form-group">
									...
									<select class="form-control select2" name="id_siswa" id="id_siswa" data-placeholder="Select Please">
										<?php foreach ($siswa as $s) { ?>
											<option></option>
												<?php if($m->id_tahun_ajaran != $s->id_tahun_ajaran ) { ?>
													<option value="<?php echo $s->id_siswa; ?>"><?php echo $s->nis; ?> - <?php echo $s->nama_siswa; ?>
													</option>
													
												<?php } ?>
												<?php if($m->id_tahun_ajaran == $s->id_tahun_ajaran ) { ?>
													<?php if($s->id_tahun_ajaran == 0 ) { ?>
													<option value="<?php echo $s->id_siswa; ?>"><?php echo $s->nis; ?> - <?php echo $s->nama_siswa; ?>
													</option>
													<?php } ?>
												<?php } ?>
											...
											?>
												<tr>
													...
													<td class="text-center" style="min-width:230px;">
														<a class="btn btn-danger" href="<?php echo base_url(); ?>kelas_siswa/delete_siswa/<?php echo $k->id_kelas_siswa_detail; ?>/<?php echo $m->id_kelas_siswa; ?>/<?= $k->id_siswa; ?>" onclick="return confirm('Anda yakin akan menghapus data ini...!')"><i class="glyphicon glyphicon-trash"></i> Delete
														</a>
													</td>
												</tr>
											<?php } ?>
